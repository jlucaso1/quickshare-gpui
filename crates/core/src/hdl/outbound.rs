...
use super::{log_transfer_summary, InnerState, State};
...
static GCM_METADATA_ENCODED: LazyLock<Vec<u8>> = LazyLock::new(|| {
    GcmMetadata {
...
pub struct OutboundRequest {
    ...
    // Reusable encoding buffer to avoid repeated allocations in encrypt_and_send
    encode_buf: Vec<u8>,
...
impl OutboundRequest {
            ...
            i = self.receiver.recv() => {
                match i {
                    ...
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        trace!("outbound: channel lagged by {n}, skipping");
                    }
                    Err(e) => {
    ...
    async fn process_ukey2_server_init(&mut self, msg: &Ukey2Message) -> Result<(), anyhow::Error> {
        ...
        let ukey_data = self
            ...
            .unwrap()
            .clone();
        self.send_frame(&ukey_data).await?;
            ...
            v1: Some(location_nearby_connections::V1Frame {
                ...
                connection_response: Some(location_nearby_connections::ConnectionResponseFrame {
					...
					os_info: Some(location_nearby_connections::OsInfo {
						r#type: Some(crate::utils::current_os_type().into())
					}),
					..Default::default()
				}),
                ..Default::default()
    ...
    async fn decrypt_and_process_secure_message(
        ...
        match v1_frame.r#type() {
            ...
            location_nearby_connections::v1_frame::FrameType::Disconnection => {
                info!("Received disconnection frame from remote");
            }
            _ => {
            ...
            sharing_nearby::connection_response_frame::Status::Accept => {
                ...
                self.update_state(
                    |e| {
                        ...
                        e.transfer_start = Some(std::time::Instant::now());
                    },
                    true,
                )
                .await;
                ...
                // Reusable read buffer — allocated once, used for all chunks
                ...
                loop {
                    let current = match ids_iter.next() {
                        Some(i) => i,
                        None => {
                            log_transfer_summary(&self.state);
                            self.update_state(
                                |e| {
                    ...
                    // Extract file info once before inner loop to avoid per-chunk clones
                    let (file_total_size, file_name_str) =
                        match self.state.transferred_files.get(&current) {
                            Some(s) => (
                                s.total_size,
                                s.file_url
                                    .file_name()
                    ...
                    let payload_header = PayloadHeader {
                        ...
                        total_size: Some(file_total_size),
                    ...
                    loop {
                        let (bytes_transferred, bytes_read) = {
                            ...
                            let bytes_read =
                                curr_state.file.as_ref().unwrap().read(&mut read_buffer)?;

                            (curr_state.bytes_transferred, bytes_read)
                        };
							...
							v1: Some(location_nearby_connections::V1Frame {
								...
								payload_transfer: Some(PayloadTransferFrame {
									...
									payload_chunk: Some(PayloadChunk {
										offset: Some(bytes_transferred),
										flags: Some(0),
										body: Some(read_buffer[..bytes_read].to_vec()),
									}),
                        ...
                        // Update bytes locally always, but only broadcast every 256KB
                        let new_ack = self
                            .state
                            .transfer_metadata
                            .as_ref()
                            .map_or(0, |t| t.ack_bytes)
                            + bytes_read as u64;
                        let should_notify = new_ack - self.state.last_notified_bytes >= 256 * 1024;
                        self.update_state(
                            |e| {
                                ...
                                if let Some(tmd) = e.transfer_metadata.as_mut() {
                                    tmd.ack_bytes = new_ack;
                                }
                                if should_notify {
                                    e.last_notified_bytes = new_ack;
                                }
                            },
                            should_notify,
                        )
                        ...
                        if bytes_transferred + bytes_read as i64 == file_total_size {
                            debug!(
    ...
    async fn finalize_key_exchange(
        ...
        self.encrypt_cipher = Some(Cipher::new_256(
            &client_key[..AES_256_KEY_LEN].try_into().unwrap(),
        ));
        let mut dc = Cipher::new_256(&server_key[..AES_256_KEY_LEN].try_into().unwrap());
        dc.set_auto_padding(true);
        self.decrypt_cipher = Some(dc);

        // Cache HMAC instances — use reset() per message instead of new_from_slice()
        self.send_hmac = Some(HmacSha256::new_from_slice(&client_hmac_key)?);
        self.recv_hmac = Some(HmacSha256::new_from_slice(&server_hmac_key)?);
    ...
    async fn encrypt_and_send(&mut self, frame: &OfflineFrame) -> Result<(), anyhow::Error> {
        ...
        // Encode D2D message into reusable buffer (avoids alloc growth after first call)
        ...
        let hb = HeaderAndBody {
            ...
            header: Header {
                ...
                iv: Some(iv.to_vec()),
                public_metadata: Some(GCM_METADATA_ENCODED.clone()),
                ..Default::default()
            },
        };
        self.encode_buf.clear();
    ...
    async fn send_frame(&mut self, data: &[u8]) -> Result<(), anyhow::Error> {
        let length_bytes = (data.len() as u32).to_be_bytes();

        self.socket.write_all(&length_bytes).await?;
        ...
        F: FnOnce(&mut InnerState),
    {
        ...
        // Yield to let the runtime process channel messages
