...
impl InboundRequest {
            ...
            i = self.receiver.recv() => {
                match i {
                    ...
                    Err(broadcast::error::RecvError::Lagged(n)) => {
                        trace!("inbound: channel lagged by {n}, skipping");
                    }
                    Err(e) => {
    ...
    pub async fn _handle(&mut self, length_buf: [u8; 4]) -> Result<(), anyhow::Error> {
        ...
        // Reuse receive buffer, growing only when needed
        self.recv_buf.resize(msg_length, 0);
        stream_read_exact(&mut self.socket, &mut self.recv_buf[..msg_length]).await?;
        ...
        match current_state.state {
            State::Initial => {
                ...
                let frame = location_nearby_connections::OfflineFrame::decode(
                    &self.recv_buf[..msg_length],
                )?;
            ...
            State::ReceivedConnectionRequest => {
                ...
                let msg = Ukey2Message::decode(&self.recv_buf[..msg_length])?;
                self.process_ukey2_client_init(&msg).await?;

                let frame_data_owned = self.recv_buf[..msg_length].to_vec();
                self.update_state(
                    |e: &mut InnerState| {
                        e.state = State::SentUkeyServerInit;
                        e.client_init_msg_data = Some(frame_data_owned);
                    },
                    false,
                )
                .await;
            ...
            State::SentUkeyServerInit => {
                ...
                let frame_data = self.recv_buf[..msg_length].to_vec();
                let msg = Ukey2Message::decode(frame_data.as_slice())?;
            ...
            _ => {
                ...
                let smsg = SecureMessage::decode(&self.recv_buf[..msg_length])?;
                    ...
                    payload_header::PayloadType::File => {
                        ...
                        if !chunk.body().is_empty() {
                            ...
                            let new_ack = self
                                ...
                                + chunk_size as u64;
                            let should_notify =
                                new_ack - self.state.last_notified_bytes >= 256 * 1024;
                            self.update_state(
                                |e| {
                                    if let Some(tmd) = e.transfer_metadata.as_mut() {
    ...
    async fn finalize_key_exchange(
        ...
        self.recv_hmac = Some(HmacSha256::new_from_slice(&client_hmac_key)?);
