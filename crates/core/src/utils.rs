...
pub fn current_os_type() -> crate::location_nearby_connections::os_info::OsType {
    #[cfg(target_os = "windows")]
    {
        crate::location_nearby_connections::os_info::OsType::Windows
    }
    #[cfg(target_os = "macos")]
    {
        crate::location_nearby_connections::os_info::OsType::Apple
    }
    #[cfg(target_os = "linux")]
    {
        crate::location_nearby_connections::os_info::OsType::Linux
    }
    #[cfg(not(any(target_os = "windows", target_os = "macos", target_os = "linux")))]
    {
        crate::location_nearby_connections::os_info::OsType::UnknownOsType
    }
}

/// Collects all local non-loopback IPv4 addresses as `IpAddr`.
/// Used to explicitly populate mDNS ServiceInfo addresses because mdns-sd
/// v0.18's `enable_addr_auto()` includes IPv6 which breaks Nearby Share discovery.
pub fn local_ipv4_addrs() -> Vec<std::net::IpAddr> {
    get_if_addrs()
        .unwrap_or_default()
        .into_iter()
        .filter_map(|iface| match iface.ip() {
            ip @ std::net::IpAddr::V4(v4) if !v4.is_loopback() => Some(ip),
            _ => None,
        })
        .collect()
}
...
pub fn is_not_self_ip(ip_address: &Ipv4Addr) -> bool {
    // When the `allow-self-discovery` feature is active, skip the self-IP
    // check so two instances on the same machine can discover each other.
    if cfg!(feature = "allow-self-discovery") {
        return true;
    }
